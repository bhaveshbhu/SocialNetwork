...
while($cdrow=mysql_fetch_array($cdresult))
  ...
  $noteid=$cdrow['stica_id'];
	...
	$time2=$cdrow['time'];
	$time3=date( "g:i a F j, Y", strtotime($time2) + 45300 );
	$date = strtotime($time2);
	$difference = time()- $date;
	if($difference>432000){	$settime=1;}else{$settime=0;}
	$time=$time3;
	$uid=$cdrow['from_id'];
	$from_id=$uid;
	$type=$cdrow['type'];
	...
	<a style='font-size:11px; background:transparent;font-family:segoe print,tahoma, verdana, arial; width:15px; float:right; text-align:right; cursor:pointer; margin-right:5px; ' class=\"close\" name='$noteid' >X</a>
		...
		<p style='color:red;font-size:9px; margin-right:2px;margin-bottom:2px; text-align:right; margin-top:2px;'>";?><?php if($settime==1){echo"$time</p>";}else{echo"<abbr class='timeago' title='$time'></abbr></p>";}?>
		<?php echo"</div>";
	echo "</br>"; 
	}
	mysql_close();?>
