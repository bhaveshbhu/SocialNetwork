...
<?php
		...
		$qalbum= "SELECT albumname, timestamp, uid  from album WHERE userid=".$usr." ORDER BY timestamp";
        //echo $q." ".$imag." ".$usr;
       // $result4 = mysql_query($q) or die(mysql_error());
	   	$result5 = mysql_query($qalbum) or die (mysql_error());
		//$ct = mysql_num_rows($result4);
		$albumcount = mysql_num_rows($result5);
...
<body>
	...
	<div id=albumhead> <span style="margin-left: 30px;">Your Albums  [<?php echo $albumcount; ?>]</span> </div>
	<div id=addmore> <span href="upload.php<?php echo "?id=$usr" ?>" class='nyroModal'><a class="btn add"> Add Album </a> </span></div>
	<div id="albumbody">
	...
	<?php 
		while($row = mysql_fetch_array($result5, MYSQL_ASSOC))
		{
		echo "<div id='albumcont' > <a href='pics.php?id={$row['uid']}'><img src='default.jpg' id='imgcont' ></img> </a><div id='albumbox'><a>  {$row['albumname']}</a> </br> <a style='font-size:9px;'>Created on : {$row['timestamp']}</a> </div></div>";
